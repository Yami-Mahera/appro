...
const FournisseursAdvanced: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...formData,
        delai_livraison_moyen: formData.delai_livraison_moyen ? parseInt(formData.delai_livraison_moyen) : undefined
      };
  ...
  const resetForm = () => {
    setFormData({
      nom: '', code_fournisseur: '', adresse: '', ville: '', code_postal: '',
      pays: 'France', telephone: '', email: '', site_web: '', conditions_paiement: '',
      delai_livraison_moyen: '', contacts: []
    });
  };
  ...
  const handleEdit = (fournisseur: Fournisseur) => {
    ...
    setFormData({
      ...
      delai_livraison_moyen: fournisseur.delai_livraison_moyen?.toString() || '',
  ...
  const addContact = () => {
    setFormData({
      ...formData,
      contacts: [...formData.contacts, { nom: '', prenom: '', telephone: '', email: '', poste: '' }]
    });
  };
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Délai livraison moyen (jours)</label>
                  <input
                    type="number"
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{viewingFournisseur.code_fournisseur}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900">
                  {viewingFournisseur.adresse}<br/>
                  {viewingFournisseur.code_postal} {viewingFournisseur.ville}<br/>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">{viewingFournisseur.telephone || 'Non renseigné'}</p>
                </div>
                <div>
                  <span className="text-sm font-medium text-gray-500">Email:</span>
                  <p className="text-gray-900">{viewingFournisseur.email || 'Non renseigné'}</p>
                </div>
              </div>
              ...
              {viewingFournisseur.site_web && (
                <div>
                  ...
                  <p className="text-gray-900">
                    <a href={viewingFournisseur.site_web} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {viewingFournisseur.site_web}
                    </a>
                  </p>
                </div>
              ...
              {viewingFournisseur.conditions_paiement && (
                <div>
                  <span className="text-sm font-medium text-gray-500">Conditions de paiement:</span>
                  <p className="text-gray-900">{viewingFournisseur.conditions_paiement}</p>
                </div>
              )}

              {viewingFournisseur.delai_livraison_moyen && (
                <div>
                  <span className="text-sm font-medium text-gray-500">Délai de livraison moyen:</span>
                  <p className="text-gray-900">{viewingFournisseur.delai_livraison_moyen} jours</p>
                </div>
              )}

              {viewingFournisseur.contacts && viewingFournisseur.contacts.length > 0 && (
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    {viewingFournisseur.contacts.map((contact, index) => (
                      <div key={index} className="bg-gray-50 p-3 rounded">
                        ...
                        {contact.poste && <p className="text-sm text-gray-600">{contact.poste}</p>}
                        {contact.telephone && <p className="text-sm">{contact.telephone}</p>}
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
                <div>
                  ...
                  <p>{new Date(viewingFournisseur.created_at).toLocaleDateString('fr-FR')}</p>
                </div>
                <div>
                  <span className="font-medium">Modifié le:</span>
                  <p>{new Date(viewingFournisseur.updated_at).toLocaleDateString('fr-FR')}</p>
                </div>
              </div>
            </div>
          </div>
